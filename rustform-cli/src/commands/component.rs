...
impl ComponentCommand {
    pub async fn execute(&self) -> Result<(), CliError> {
        match &self.action {
            ...
            ComponentAction::Compatibility { uri_or_path, version } => {
                self.check_compatibility(uri_or_path, version.as_deref()).await
            }
            ComponentAction::Test { component, directory, generate_test_app, unit_tests_only, skip_compatibility } => {
                self.test_component(component, directory, *generate_test_app, *unit_tests_only, *skip_compatibility).await
            }
        }
    }
    ...
    async fn validate_component(&self, path: &PathBuf) -> Result<(), CliError> {
        ...
        println!("     API Version: {}", manifest.api_compatibility.api_version);
        println!("     Min Version: {}", manifest.api_compatibility.min_version);
        ...
        println!("   Templates: {}", manifest.provides.as_ref().map(|p| p.templates.len()).unwrap_or(0));
        println!("   Assets: {}", manifest.provides.as_ref().map(|p| p.assets.len()).unwrap_or(0));
        println!("   Hooks: {}", manifest.provides.as_ref().map(|p| p.hooks.len()).unwrap_or(0));
    ...
    async fn test_component(
        ...
        let component_path = if PathBuf::from(component).exists() {
            ...
            if !path.exists() {
                return Err(CliError::ComponentError(format!("Component '{}' not found in directory '{}'", component, directory.display())));
            }
            path
        };
        ...
        let manifest_path = {
            ...
            println!("   Checking: {:?} (exists: {})", rustform_manifest, rustform_manifest.exists());
            println!("   Checking: {:?} (exists: {})", component_manifest, component_manifest.exists());
        ...
        if !skip_compatibility {
            ...
            let status = manifest.compatibility_status(rust_form_version)
                .map_err(|e| CliError::ComponentError(format!("Failed to check compatibility: {}", e)))?;
            
            if !status.is_compatible() {
                println!("   ❌ {}", status.message());
                return Err(CliError::ComponentError("Component is not compatible with current rust-form version".to_string()));
            }
            println!("   ✅ Component is compatible with rust-form v{}", rust_form_version);
        } else {
        ...
        if generate_test_app {
            ...
            self.generate_test_application(&manifest, &component_path).await?;
        ...
        let quality_metrics = self.assess_component_quality(&manifest, &component_path).await?;
    ...
    async fn run_component_unit_tests(&self, component_path: &PathBuf) -> Result<ComponentTestResults, CliError> {
                ...
                if let Ok(entries) = fs::read_dir(&test_dir) {
                    for entry in entries.flatten() {
                        ...
                        if path.is_file() {
                            ...
                            if file_name.ends_with("_test.rs") || file_name.ends_with(".test.js") || file_name.starts_with("test_") {
        ...
        if cargo_toml.exists() {
            ...
            match self.run_cargo_test(&component_path).await {
                Ok((passed, failed)) => {
                    ...
                    println!("   ✅ Rust tests completed: {} passed, {} failed", passed, failed);
                }
                Err(e) => {
        ...
        if package_json.exists() {
            ...
            match self.run_npm_test(&component_path).await {
                Ok((passed, failed)) => {
                    results.unit_tests_passed += passed;
                    results.unit_tests_failed += failed;
                    println!("   ✅ JS/TS tests completed: {} passed, {} failed", passed, failed);
                }
                Err(e) => {
    ...
    async fn run_cargo_test(&self, component_path: &PathBuf) -> Result<(usize, usize), CliError> {
        ...
        if !manifest_path.exists() {
            return Err(CliError::ComponentError(format!("Cargo.toml not found at: {}", manifest_path.display())));
        }
        ...
        println!("   Running: cargo test --manifest-path {} (from {})", manifest_path.display(), absolute_component_path.display());
        ...
        if output.status.success() {
            ...
            if passed == 0 && failed == 0 {
                passed = stdout.matches("test ").filter(|line| line.contains(" ... ok")).count();
                failed = stdout.matches("test ").filter(|line| line.contains(" ... FAILED")).count();
            }
            
            Ok((passed, failed))
        } else {
            Err(CliError::ComponentError(format!("Cargo test failed: {}\n{}", stderr, stdout)))
        }
    }
    ...
    async fn run_npm_test(&self, component_path: &PathBuf) -> Result<(usize, usize), CliError> {
        ...
        if output.status.success() {
            ...
            let stderr = String::from_utf8_lossy(&output.stderr);
            Err(CliError::ComponentError(format!("npm test failed: {}", stderr)))
        }
    }
    
    async fn generate_test_application(&self, manifest: &rustform_core::component::ComponentManifest, component_path: &PathBuf) -> Result<(), CliError> {
        let test_app_path = component_path.join("test-app");
        
        // Create test application directory
        if test_app_path.exists() {
            fs::remove_dir_all(&test_app_path)
                .map_err(|e| CliError::ComponentError(format!("Failed to clean test app directory: {}", e)))?;
        }
        
        fs::create_dir_all(&test_app_path)
            .map_err(|e| CliError::ComponentError(format!("Failed to create test app directory: {}", e)))?;
...
"#, manifest.name, manifest.name, manifest.name);
        ...
        let output = std::process::Command::new("rustform")
            .arg("generate")
            .arg(config_path)
            .arg("--output")
            .arg(&test_app_path)
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    match build_output {
                        Ok(build_result) => {
                            if build_result.status.success() {
                                ...
                                println!("   ⚠️  Test application compilation warnings: {}", stderr);
                            }
                        }
                        Err(e) => {
                    ...
                    return Err(CliError::ComponentError(format!("Failed to generate test application: {}", stderr)));
                }
            }
            Err(e) => {
                return Err(CliError::ComponentError(format!("Failed to run rustform generate: {}", e)));
            }
        }
    ...
    async fn assess_component_quality(&self, manifest: &rustform_core::component::ComponentManifest, component_path: &PathBuf) -> Result<ComponentQualityMetrics, CliError> {
        let mut metrics = ComponentQualityMetrics {
            has_documentation: false,
            has_examples: false,
            has_tests: false,
            template_count: manifest.provides.as_ref().map(|p| p.templates.len()).unwrap_or(0),
            asset_count: manifest.provides.as_ref().map(|p| p.assets.len()).unwrap_or(0),
            hook_count: manifest.provides.as_ref().map(|p| p.hooks.len()).unwrap_or(0),
            quality_score: 0.0,
        };
        ...
        if manifest.description.is_some() { score += 10.0; }
        if manifest.author.is_some() { score += 5.0; }
        if manifest.license.is_some() { score += 5.0; }
        if manifest.repository.is_some() { score += 5.0; }
        
        // Score from content
        if metrics.has_documentation { score += 20.0; }
        if metrics.has_examples { score += 15.0; }
        if metrics.has_tests { score += 25.0; }
        ...
        println!("   📋 Documentation: {}", if metrics.has_documentation { "✅" } else { "❌" });
        println!("   📚 Examples: {}", if metrics.has_examples { "✅" } else { "❌" });
        println!("   🧪 Tests: {}", if metrics.has_tests { "✅" } else { "❌" });
    ...
    fn print_test_summary(&self, component_name: &str, results: &ComponentTestResults) {
        ...
        if total_tests > 0 {
            ...
            if results.unit_tests_failed == 0 {
                ...
                println!("   Status: ❌ {} test(s) failing", results.unit_tests_failed);
            }
    ...
    async fn check_compatibility(&self, uri_or_path: &str, version: Option<&str>) -> Result<(), CliError> {
        ...
        let manifest = if path.exists() {
            ...
            let manifest_path = if path.is_file() {
                ...
                if filename == "rustform-component.yml" || filename == "component.yml" {
                    ...
                    return Err(CliError::ComponentError("File must be rustform-component.yml or component.yml".to_string()));
                }
            } else if path.is_dir() {
                ...
                return Err(CliError::ComponentError("Path must be a directory or manifest file".to_string()));
            };
            ...
            component_system.fetch_manifest(&component_uri).await
                .map_err(|e| CliError::ComponentError(format!("Failed to fetch manifest: {}", e)))?
        } else {
            return Err(CliError::ComponentError("Invalid path or URI format".to_string()));
        };
        
        println!("📋 Component: {} v{}", manifest.name, manifest.version);
        println!("🔧 Checking compatibility with rust-form v{}", rust_form_version);
        println!();
        ...
        if !status.is_compatible() {
            ...
            println!("💡 Suggestions:");
            match status {
                rustform_core::component::CompatibilityStatus::TooOld { current, required_min } => {
                    println!("   • Upgrade rust-form from {} to {} or higher", current, required_min);
                    println!("   • Or find an older version of this component");
                }
                rustform_core::component::CompatibilityStatus::TooNew { current, supported_max } => {
            ...
            return Err(CliError::ComponentError("Component is not compatible".to_string()));
        }
