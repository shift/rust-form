...
impl GenerateCommand {
    ...
    pub async fn execute(&self) -> Result<(), CliError> {
        ...
        let config = parse_config_file(&self.config).map_err(|e| {
            ...
            match e {
                ...
                rustform_core::ConfigError::Io(io_error) => CliError::io_error(io_error),
            }
        })?;
        ...
        let mut pipeline = rustform_codegen::GenerationPipeline::new().map_err(|e| {
            CliError::generation_error(format!("Failed to initialize pipeline: {}", e))
        })?;

        let generated_project = pipeline
            .generate(&config, &output_dir)
            .await
            .map_err(|e| CliError::generation_error(format!("Generation failed: {}", e)))?;

        // Create output directory
        std::fs::create_dir_all(&output_dir).map_err(|e| CliError::io_error(e))?;
        ...
        for file in &generated_project.files {
            ...
            if let Some(parent) = file_path.parent() {
                std::fs::create_dir_all(parent).map_err(|e| CliError::io_error(e))?;
            }

            std::fs::write(&file_path, &file.content).map_err(|e| CliError::io_error(e))?;
