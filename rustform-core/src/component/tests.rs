use crate::component::manifest::{ApiCompatibility, ComponentDependencies};
use crate::component::{CompatibilityStatus, ComponentManifest, ComponentSystem, ComponentUri};
use rstest::*;
...
"#;
    ...
    let manifest: ComponentManifest =
        serde_yaml::from_str(yaml).expect("Failed to parse component manifest");

    assert_eq!(manifest.name, "jwt-auth");
    assert_eq!(
        manifest.description,
        Some("JWT authentication component".to_string())
    );
    assert_eq!(manifest.api_compatibility.api_version, "0.1.0");
    assert_eq!(manifest.api_compatibility.min_version, "0.1.0");
    assert_eq!(
        manifest.api_compatibility.max_version,
...
fn test_component_uri_parsing() {
    // Test GitHub URI (simplified for now)
    let uri_str = "github://owner/repo@1.0.0";
    match uri_str.parse::<ComponentUri>() {
        Ok(_uri) => {
            // URI parsing works
        }
        Err(_) => {
            // For now, just ensure we handle errors gracefully
        }
    }

    // Test local file URI (simplified for now)
    ...
    // Test registry URI (simplified for now)
...
async fn test_component_system_creation() {
    ...
    let system = ComponentSystem::with_version(custom_version.to_string())
        .expect("Should create component system with custom version");
...
fn create_test_manifest() -> ComponentManifest {
    ComponentManifest {
        ...
        description: Some("JWT token management".to_string()),
    ...
    proptest! {
        ...
        fn test_api_version_compatibility_property(
            ...
            if let Ok(status) = status {
                let is_compatible = matches!(status, CompatibilityStatus::Compatible { .. });
...
"#;
        ...
        let manifest: ComponentManifest =
            serde_yaml::from_str(&file_content).expect("Failed to parse manifest");

        assert_eq!(manifest.name, "test-component");
        assert_eq!(manifest.category.as_deref(), Some("database"));
        assert!(manifest.validate().is_ok());
    }
...
fn test_gdpr_component_parsing() {
    let gdpr_content = std::fs::read_to_string(
        "./components/backend/compliance/gdpr-data-subject-rights/component.yml",
    )
    ...
    match ComponentManifest::from_yaml(&gdpr_content) {
        Ok(manifest) => {
            ...
            println!(
                "   ✅ Rust dependencies: {}",
                manifest.dependencies.rust.len()
            );
