...
impl ConfigParser {
    ...
    pub fn parse(&self) -> ConfigResult<Config> {
        ...
        let yaml_value: Value = serde_yaml::from_str(&self.source)
            .map_err(|e| self.enhance_yaml_error(e))?;
        ...
        crate::config::validate_config(&config)
            .map_err(|validation_error| {
                self.enhance_validation_error(validation_error)
            })?;
    ...
    fn find_field_location(&self, error: &ValidationError) -> Option<(usize, usize)> {
        ...
        match error {
            ValidationError::MissingField { field } => {
                self.find_text_location(field)
            }
            ValidationError::ModelNotFound { model } => {
                self.find_text_location(model)
            }
            ValidationError::InvalidPath { path, .. } => {
                self.find_text_location(path)
            }
            ValidationError::NoPrimaryKey { model } => {
                self.find_text_location(model)
            }
            _ => None,
        }
