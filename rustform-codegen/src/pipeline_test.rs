...
mod component_system_tests {
    ...
    use rustform_core::{
        component::{ComponentSystem, ComponentUri},
        Config,
    };
    use std::fs;
...
"#;
        ...
        fs::write(
            component_path.join("rustform-component.yml"),
            manifest_content,
        )
        ...
        let mut component_system =
            ComponentSystem::new().expect("Failed to create component system");
        let uri: ComponentUri = format!("path:{}", component_path.display())
            .parse()
            .expect("Failed to parse URI");

        let component = component_system
            .install_component(&uri)
            .await
        ...
        assert_eq!(component.manifest.provides.as_ref().unwrap().templates.len(), 1);
    }
...
"#;
        ...
        // Test generation
        let mut pipeline = GenerationPipeline::new().expect("Failed to create pipeline");
