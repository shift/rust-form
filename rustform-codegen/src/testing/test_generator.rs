...
impl TestGenerator {
    ...
    pub fn generate_tests(&self, config: &ComponentConfig) -> Result<HashMap<String, String>, TestGenerationError> {
    ...
    fn generate_unit_test(&self, config: &ComponentConfig, test_name: &str) -> Result<String, TestGenerationError> {
    ...
    fn generate_integration_test(&self, config: &ComponentConfig, test_name: &str) -> Result<String, TestGenerationError> {
        let mut context = Context::new();
        context.insert("component", config);
        context.insert("test_name", test_name);
    ...
    fn generate_performance_test(&self, config: &ComponentConfig, test_name: &str) -> Result<String, TestGenerationError> {
        let mut context = Context::new();
        context.insert("component", config);
        context.insert("test_name", test_name);
    ...
    /// Generate test utilities
    fn generate_test_utilities(&self, config: &ComponentConfig) -> Result<String, TestGenerationError> {
        let mut context = Context::new();
        context.insert("component", config);
        
        self.tera.render("test_utilities.rs.tera", &context)
            .map_err(TestGenerationError::TemplateError)
    }
    ...
    fn generate_mocks(&self, config: &ComponentConfig) -> Result<String, TestGenerationError> {
        ...
        self.tera.render("mocks.rs.tera", &context)
            .map_err(TestGenerationError::TemplateError)
    }
...
fn test_name_filter(value: &tera::Value, _: &HashMap<String, tera::Value>) -> tera::Result<tera::Value> {
    let test_name = value.as_str().ok_or_else(|| {
        tera::Error::msg("test_name filter can only be applied to strings")
    })?;
...
fn mock_name_filter(value: &tera::Value, _: &HashMap<String, tera::Value>) -> tera::Result<tera::Value> {
    let name = value.as_str().ok_or_else(|| {
        tera::Error::msg("mock_name filter can only be applied to strings")
    })?;
    
    let mock_name = format!("Mock{}", 
        name.split('_')
            .map(|word| {
...
fn rust_type_filter(value: &tera::Value, _: &HashMap<String, tera::Value>) -> tera::Result<tera::Value> {
    let type_str = value.as_str().ok_or_else(|| {
        tera::Error::msg("rust_type filter can only be applied to strings")
    })?;
...
mod tests {
    ...
    fn create_test_component() -> ComponentConfig {
        ComponentConfig {
            ...
            dependencies: DependencyConfig {
                ...
                test_deps: Some(vec![
                    "tokio-test = \"0.4\"".to_string(),
                ]),
                nix: NixDependencies {
                    buildInputs: vec!["openssl".to_string()],
                    nativeBuildInputs: vec!["rustc".to_string()],
                    devShell: DevShellConfig {
                        packages: vec!["rust-analyzer".to_string()],
                    },
                },
                flake_inputs: None,
            },
            templates: TemplateConfig {
                generates: vec!["auth/token_handler.rs".to_string()],
                requires: None,
            },
            tests: TestConfig {
                unit: vec![
                ...
                integration: Some(vec![
                    "test_auth_flow".to_string(),
                ]),
                performance: Some(vec![
                    "benchmark_token_operations".to_string(),
                ]),
            },
            documentation: DocumentationConfig {
                examples: vec!["basic_usage.rs".to_string()],
                api_docs: true,
                tutorial: true,
            },
            config_schema: None,
