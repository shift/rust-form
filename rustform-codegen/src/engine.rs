...
use include_dir::Dir;
use rustform_core::component::{ComponentLockfile, ComponentSystem, ComponentUri};
...
impl TemplateEngine {
    pub fn new() -> Result<Self, CodeGenError> {
                ...
                if let Some(path) = file.path().to_str() {
                    if path.ends_with(".tera") {
                        let content = file.contents_utf8().ok_or_else(|| {
                            CodeGenError::Template(format!("Template {} is not valid UTF-8", path))
                        })?;
                        tera.add_raw_template(path, content).map_err(|e| {
        ...
        let component_system = ComponentSystem::new().map_err(|e| {
            CodeGenError::Template(format!("Failed to initialize component system: {}", e))
        })?;
    ...
    pub fn add_template(&mut self, name: &str, content: &str) -> Result<(), CodeGenError> {
        self.tera.add_raw_template(name, content).map_err(|e| {
            CodeGenError::Template(format!("Failed to add template '{}': {}", name, e))
        })
    }
    ...
    pub async fn install_components(&mut self, config: &Config) -> Result<(), CodeGenError> {
        if let Some(components) = &config.components {
            ...
            for (component_name, component_spec) in components {
                ...
                let component_uri = ComponentUri::from_str(component_spec).map_err(|e| {
                    CodeGenError::Template(format!(
                        "Invalid component URI '{}': {}",
                        component_spec, e
                    ))
                })?;
                ...
                for (template_name, template_content) in &component.content.templates {
                    let full_template_name =
                        format!("components/{}/{}", component_name, template_name);
                    self.tera
                        .add_raw_template(&full_template_name, template_content)
                        .map_err(|e| {
                            CodeGenError::Template(format!(
                                "Failed to add component template '{}': {}",
                                full_template_name, e
                            ))
                        })?;
                }
    ...
    pub async fn generate_with_components(
        ...
        lockfile
            .save(output_dir.join("rustform.lock"))
    ...
    pub async fn generate_lockfile(
        &mut self,
        config: &Config,
    ) -> Result<ComponentLockfile, CodeGenError> {
        ...
        if let Some(components) = &config.components {
            for (component_name, component_spec) in components {
                ...
                let locked_component = rustform_core::component::LockedComponent {
                    ...
                    version: component_uri
                        .version
                        .clone()
                        .unwrap_or_else(|| "latest".to_string()),
                    resolved: component_uri.resolve_url().map_err(|e| {
                        CodeGenError::Template(format!(
                ...
                lockfile
                    .dependencies
                    .insert(component_name.clone(), locked_component);
            }
        }
    ...
    pub async fn validate_components(
        ...
        if let Some(components) = &config.components {
            for (component_name, component_spec) in components {
                ...
                match self
                    .component_system
                    .check_availability(&component_uri)
                    .await
                {
                    Ok(true) => {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to check availability of component '{}': {}",
                            component_name, e
                        );
                        missing_components.push(component_name.clone());
                    }
    ...
    pub fn register_component_helpers(&mut self) {
        ...
        self.tera
            .register_function("component_template", component_template_function);
        self.tera
            .register_function("component_asset", component_asset_function);
        self.tera
            .register_function("list_components", list_components_function);

        // Register component-related filters
        self.tera
            .register_filter("component_path", component_path_filter);
    }
}
...
mod tests {
    ...
    fn test_component_template_function() {
        ...
        args.insert(
            "template".to_string(),
            Value::String("button.tera".to_string()),
        );
